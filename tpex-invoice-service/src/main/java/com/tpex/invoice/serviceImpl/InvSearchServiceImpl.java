...
public class InvSearchServiceImpl implements InvSearchService {
	...
	public SearchInvHaisenDetailResponse fetchHaisenDetail(
		...
		try {
			...
			{    
				...
				List<Object[]> feederDeails = noemHaisenDtlsRepository.getfeederDetailsForHaisenNo(dt.getHaisenNo(),dt.getHaisenYearMonth().replace("/", ""),buyer,regular, cpo, spo);
			...
			resp.setSearchInvHaisenDetailResponse(resultDto);
	 ...
	 */
	@Override
	public SearchByInvNoResponseDto searchByInvNo(String envNo) throws Exception {
		...
		if (currencyCode != null) {
			...
			if (oemCurrencyMstEntity.isPresent() && oemCurrencyMstEntity.get() != null
				...
				response.setCurrencyCode(oemCurrencyMstEntity.get().getCrmCd() + "-" + oemCurrencyMstEntity.get().getCrmDesc());
			}
		}
		if (payTermCode != null) {
			...
			if (oemPmntTermMstEntity.isPresent() && oemPmntTermMstEntity.get() != null
				...
				response.setPaymentTerm(oemPmntTermMstEntity.get().getPtCd() + "-" + oemPmntTermMstEntity.get().getPtDesc());
			}
		}
		...
		if (productGrpCode != null) {
			...
			if (insProdGrpMstEntity.isPresent() && insProdGrpMstEntity.get() != null
				...
				response.setProductGrp(insProdGrpMstEntity.get().getIpgProdGrpCd() + "-" + insProdGrpMstEntity.get().getIpgProdGrpDesc());
			}
		}
	...
	private static SearchByInvNoResponseDto getSearchByInvNoResponseDto( Optional<InsInvDtlsEntity> insInvDetailsEntity) {
	 ...
	 */
	@Override
	public InvoiceDetailsResponseWrapper getInvoiceDetails(String haisenNo, String haisenYear) throws Exception {
	...
	private static void populatePartDetails(Map<String, String> map, List<OemPortMstEntity> emPortMstObj, List<PortGrpObjDto> listdto) {
	...
	public void updateInvDetailsByInvNo(UpdateInvDetailsRequestDTO updateRequest) {
		...
		if (productGroup != null && !productGroup.equals("") && paymentTerm != null && !paymentTerm.equals("") && custCode != null && !custCode.equals("")) {
			...
			if (insInvDtlsEntityOptional.isPresent()) {
				...
				insInvDtlsEntity.setIndFreight(updateRequest.getFreight().isBlank() ? Double.valueOf(0) : Double.valueOf(updateRequest.getFreight().replace(",", "")));
				insInvDtlsEntity.setIndInsurance(updateRequest.getInsurance().isBlank() ? Double.valueOf(0) : Double.valueOf(updateRequest.getInsurance().replace(",", "")));
	...
	/* pins003 Anurag Tpex 189
	 * This Method produces Invoice Attached Sheet Details based 
	 * on Company_code & User_ID & Incoice_no.	 * 
	 * 
	 * */

	public Object downloadPINS103(String cmpCd, String templateId, String invoiceNo, String userId, String reportFormat) throws Exception {
		...
		List<invAttachedSheetResponseDto> response = querydataforpins003.stream().map(t -> new invAttachedSheetResponseDto(
		...
		config.put(ConstantUtils.REPORT_DIRECTORY, tpexConfigRepository.findByName(ConstantUtils.INCVOICE_GENERATION_REPORT_DIRECTORY).getValue()); //"reportDirectory" //"invoiceGeneration.report.directory"
		config.put(ConstantUtils.REPORT_FORMAT, tpexConfigRepository.findByName(ConstantUtils.INVOICE_GENERATION_REPORT_FORMAT).getValue());//"reportFormat" //invoiceGeneration.report.format"
		config.put(ConstantUtils.REPORT_SIZE_LIMIT,
				tpexConfigRepository.findByName(ConstantUtils.INVOICE_GENERATION_REPORT_SIZE_LIMIT).getValue());  //"reportSizeLimit" //"invoiceGeneration.report.size.limit"
