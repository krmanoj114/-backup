...
class Binf009DeleteStagingDataTest {
	
	@InjectMocks 
	private Binf009DeleteStagingData binf009DeleteStagingData;
	...
	void executeTest() throws Exception {
		...
		ReflectionTestUtils.setField(binf009DeleteStagingData, "compCode", "TMT", String.class);
		
		binf009DeleteStagingData.execute(contribution, chunkContext);
		
		verify(binf009CustomRepository, times(1)).deleteStagingData(anyString());
	...
	void beforeStepInsertLogTest() {
		...
		binf009DeleteStagingData.beforeStep(stepExecution);
	...
	void beforeStepAfterErrTest() {
		...
		when(jobExecution.getExecutionContext()).thenReturn(executionContext);
		when(executionContext.getString(GlobalConstants.COMP_CODE)).thenReturn("compCode");
		when(stepExecution.getStepName()).thenReturn("BINF009DeleteStagingDataAfterError");
	...
	@Test
	void afterStepTest() {
		when(stepExecution.getStepName()).thenReturn("BINF009DeleteStagingData");
		
		ExitStatus afterStep = binf009DeleteStagingData.afterStep(stepExecution);
