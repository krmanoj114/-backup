...
public interface InvoiceCoverPageRepository extends JpaRepository<RddDownLocDtlEntity, Integer>{
	...
	@Query(value="SELECT MAX(CASE WHEN PARA_CD = 'COUNTRY' THEN PARA_VAL ELSE NULL END) as COUNTRY_NM,"+
			"MAX(CASE WHEN PARA_CD = 'CMP_CD' THEN PARA_VAL ELSE NULL END) as CMP_CD,"+
			"MAX(CASE WHEN PARA_CD = 'XDOC_CNT' THEN PARA_VAL ELSE NULL END) as XDOC_CNT "+
			"FROM TB_M_PARAMETER WHERE PARA_CD IN ('COUNTRY','CMP_CD','XDOC_CNT')",nativeQuery = true)
	...
	@Query(value="SELECT coalesce(LENGTH(MARK4),0),"+
			"coalesce(LENGTH(MARK5),0), "+
			"coalesce(LENGTH(MARK6),0) "+
	...
	@Query(value="SELECT  IND.INV_NO INV_NO,\r\n"
			+ "				DATE_FORMAT(IND.INV_DT,'%d/%m/%Y') INV_DT,\r\n"
			+ "				IND.CUST_NM CUST_NM,\r\n"
			...
			+ "				DATE_FORMAT(IND.ETD,'%d/%m/%Y') ETD_DT,\r\n"
			+ "				DATE_FORMAT(IND.ETA,'%d/%m/%Y') ETA_DT,\r\n"
			+ "				-- UNDER 'VESSEL DESC'\r\n"
			...
			+ "				AND     IND.CUST_CNTRY    =  FD.CNTRY_CD", nativeQuery = true)
