...
public class InvGenWorkPlanMstServiceImpl implements InvGenWorkPlanMstService {
	...
	private static final String STORE_IN_DB = "storeInDB";

	private static final String REPORT_FORMAT = "reportFormat";

	private static final String INVOICE_GENERATION_REPORT_FORMAT = "invoiceGeneration.report.format";

	private static final String INVOICE_GENERATION_REPORT_DIRECTORY = "invoiceGeneration.report.directory";

	private static final String REPORT_DIRECTORY = "reportDirectory";

	private static final String INVOICE_GENERATION_WORK_PLAN = "InvoiceGenerationWorkPlan";
	...
	@Override
	public boolean saveInvoice(List<InvGenWorkPlanMstDTO> listInvGenWorkPlanMstDTO) {
		...
		for (InvGenWorkPlanMstDTO module : listInvGenWorkPlanMstDTO) {
			...
			if (module != null && module.getId() != null) {
				id.setOriginalEtd(
						DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, module.getId().getOriginalEtd()));
				...
				Optional<InvGenWorkPlanMstEntity> optionOfInvGenWorkPlanMstEntity = invGenWorkPlanMstRepository
						.findById(id);
				if (optionOfInvGenWorkPlanMstEntity.isPresent()) {
					...
					if ("N".equalsIgnoreCase(entity.getInvGenFlag())) {
						...
						if (Util.nullCheck(module.getEtd1()))
							entity.setEtd1(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, module.getEtd1()));

						if (Util.nullCheck(module.getIssueInvoiceDate()))
							entity.setIssueInvoiceDate(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT,
									module.getIssueInvoiceDate()));
						...
						if (Util.nullCheck(module.getEta1()))
							entity.setEta1(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, module.getEta1()));

						if (Util.nullCheck(module.getEta2()))
							entity.setEta2(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, module.getEta2()));

						if (Util.nullCheck(module.getEta3()))
							entity.setEta3(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, module.getEta3()));

						if (Util.nullCheck(module.getEtd2()))
							entity.setEtd2(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, module.getEtd2()));

						if (Util.nullCheck(module.getEtd3()))
							entity.setEtd3(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, module.getEtd3()));
						...
						entity.setUpdateDate(DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT,
	...
	@Override
	public Page<InvGenWorkPlanMstEntity> fetchInvoiceDetails(int pageNo, int pageSize, String invoiceFromDate,
		...
		List<InvGenWorkPlanMstEntity> listResult = null;

		validateInvGenSearchAndDownloadRequest(invoiceFromDate, etd1fromDate, currentDateMinus6Months, dateFormat);
		...
		if (getInvoiceDateNullCheck(invoiceFromDate, etd1fromDate)) {
			if (checkDstCode(dstCode)) {
				pagedResult = invGenWorkPlanMstRepository.findByIssueInvoiceDateBetweenAndEtd1BetweenAndIdContDestIn(
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate),
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate), dstCode,
						paging);
			} else {
				if(StringUtils.isNotEmpty(invoiceToDate) && !"null".equalsIgnoreCase(invoiceToDate.trim()) && (StringUtils.isNotEmpty(etd1ToDate) && !"null".equalsIgnoreCase(etd1ToDate.trim()))) {
					pagedResult = invGenWorkPlanMstRepository.findByIssueInvoiceDateBetweenAndEtd1Between(
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate),
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate), paging);
				}else {
					if(StringUtils.isNotEmpty(invoiceToDate) && !"null".equalsIgnoreCase(invoiceToDate.trim()) && (StringUtils.isEmpty(etd1ToDate))) {

						pagedResult = invGenWorkPlanMstRepository.findByIssueInvoiceDateBetweenAndEtd1Between(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate), paging);
					}else if(StringUtils.isNotEmpty(etd1ToDate) && !"null".equalsIgnoreCase(etd1ToDate.trim()) && (StringUtils.isEmpty(invoiceToDate))){
						pagedResult = invGenWorkPlanMstRepository.findByIssueInvoiceDateBetweenAndEtd1Between(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
						...
						listResult = invGenWorkPlanMstRepository.findByGreaterThanEqualToIssueInvoiceDateAndEtd1(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate));
						pagedResult = new PageImpl<>(listResult, paging, listResult.size());
							
					}
				}
			}
		} else if (StringUtils.isNotEmpty(invoiceFromDate) && !"null".equalsIgnoreCase(invoiceFromDate.trim())
				&& (StringUtils.isEmpty(etd1fromDate)) || "null".equalsIgnoreCase(etd1fromDate.trim())) {
			if (checkDstCode(dstCode)) {
				pagedResult = invGenWorkPlanMstRepository.findByIssueInvoiceDateBetweenAndIdContDestIn(
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate), dstCode,
						paging);
			} else {
				if(StringUtils.isNotEmpty(invoiceToDate) && !"null".equalsIgnoreCase(invoiceToDate.trim())){
					pagedResult = invGenWorkPlanMstRepository.findByIssueInvoiceDateBetween(
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate), paging);
				}
				else {
					
					listResult = invGenWorkPlanMstRepository.findByGreaterthanEqualToInvoiceDateFrom(DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate));
					pagedResult = new PageImpl<>(listResult, paging, listResult.size());
			...
			if (checkDstCode(dstCode)) {
				...
				if(StringUtils.isNotEmpty(etd1ToDate) && !"null".equalsIgnoreCase(etd1ToDate.trim())){
					pagedResult = invGenWorkPlanMstRepository.findByEtd1Between(
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
							DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate), paging);
				}else {
					listResult = invGenWorkPlanMstRepository.findByGreaterthanEqualToEtdFrom(DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate));
					pagedResult = new PageImpl<>(listResult, paging, listResult.size());
	...
	private void validateInvGenSearchAndDownloadRequest(String invoiceFromDate, String etd1fromDate,
			LocalDate currentDateMinus6Months, DateTimeFormatter formatter1) {
		if ((StringUtils.isBlank(invoiceFromDate) || "null".equalsIgnoreCase(invoiceFromDate))
				&& (StringUtils.isBlank(etd1fromDate) || "null".equalsIgnoreCase(etd1fromDate))) {

			throw new InvalidInputParametersException(
					"Either Invoice Date From or ETD Date From is be required for search operation");
		} else if ((StringUtils.isNotBlank(invoiceFromDate) && !"null".equalsIgnoreCase(invoiceFromDate))
				|| (StringUtils.isNotBlank(etd1fromDate) && !"null".equalsIgnoreCase(etd1fromDate))) {
			if (StringUtils.isNotBlank(invoiceFromDate)) {
				if (LocalDate.parse(invoiceFromDate, formatter1).isBefore(currentDateMinus6Months)) {
					throw new InvalidInputParametersException(
							"Allowed to select only max 6 Month Back date from the current date");
				}

			} else {
				if (LocalDate.parse(etd1fromDate, formatter1).isBefore(currentDateMinus6Months)) {
					throw new InvalidInputParametersException(
							"Allowed to select only max 6 Month Back date from the current date");
				}
	...
	public Map<String, Integer> getCountReportDataToDownload(String invoiceFromDate, String invoiceToDate,
		...
		RddDownLocDtlEntity rddDownLocDtlEntity = rddDownLocDtlRepository
				.findTopByReportNameAndCreateByOrderByReportIdDesc(INVOICE_GENERATION_WORK_PLAN, createdBy);
		...
		config.put(REPORT_DIRECTORY, tpexConfigRepository.findByName(INVOICE_GENERATION_REPORT_DIRECTORY).getValue());
		config.put(REPORT_FORMAT, tpexConfigRepository.findByName(INVOICE_GENERATION_REPORT_FORMAT).getValue());
		config.put(STORE_IN_DB, "false");
		config.put(LOGIN_USER_ID, createdBy);

		String fileTemplateName = INVOICE_GENERATION_WORK_PLAN;
		String fileFormat = (String) config.get(REPORT_FORMAT);
		String fileName = jasperReportService.getFileName(fileTemplateName, fileFormat);
		StringBuilder sb = new StringBuilder().append(String.valueOf(config.get(REPORT_DIRECTORY))).append("/")
				.append(fileName);

		RddDownLocDtlEntity savedRddDownLocDtlEntity = jasperReportService.saveOfflineDownloadDetail(fileTemplateName,
				config, sb);
		...
		if (getInvoiceDateNullCheck(invoiceFromDate, etd1fromDate)) {
			if (checkDstCode(dstCode)) {
				...
				rowCount = invGenWorkPlanMstRepository.findCountByIssueInvDateAndEtd1(
						...
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate),
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
						DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate));
			}
	...
	@Async
	@Override
	public Object downloadReportOffline(String invoiceFromDate, String invoiceToDate, String etd1fromDate,
			String etd1ToDate, List<String> dstCode, String fileFormat, String loginUserId, int savedReportId) {
		...
		config.put(STORE_IN_DB, "true");
		config.put(LOGIN_USER_ID, loginUserId);
		...
		// return path
		jasperReportService.getJasperReportDownloadOffline(invGenWorkPlanMstEntityList, fileFormat, fileTemplateName,
				parameters, config, reportId, sb);
	...
	private String getFileFormat(String fileFormat, Map<String, Object> config) {
		fileFormat = (fileFormat == null || fileFormat.equals("")) ? (String) config.get(REPORT_FORMAT) : fileFormat;
		return fileFormat;
	}

	@Override
	public Object downloadReportOnline(String invoiceFromDate, String invoiceToDate, String etd1fromDate,
			String etd1ToDate, List<String> dstCode, String fileFormat, String loginUserId)
		...
		jasperResponse = jasperReportService.getJasperReportDownloadOnline(invGenWorkPlanMstEntityList, fileFormat,
	...
	private List<InvGenWorkPlanMstEntity> getReportDataToDownload(String invoiceFromDate, String invoiceToDate,
			...
			List<InvGenWorkPlanMstEntity> invGenWorkPlanMstEntityList) {
		if (getInvoiceDateNullCheck(invoiceFromDate, etd1fromDate)) {
			if (checkDstCode(dstCode)) {
				invGenWorkPlanMstEntityList = invGenWorkPlanMstRepository
						.findByIssueInvoiceDateBetweenAndEtd1BetweenAndIdContDestInOrderByIssueInvoiceDateAscEtd1AscIdContDestAscIdLinerAscBrokerAscIdRenbanCodeAsc(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate),
				...
				invGenWorkPlanMstEntityList = invGenWorkPlanMstRepository
						.findByIssueInvoiceDateBetweenAndEtd1BetweenOrderByIssueInvoiceDateAscEtd1AscIdContDestAscIdLinerAscBrokerAscIdRenbanCodeAsc(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate));
			...
			if (checkDstCode(dstCode)) {
				invGenWorkPlanMstEntityList = invGenWorkPlanMstRepository
						.findByIssueInvoiceDateBetweenAndIdContDestInOrderByIssueInvoiceDateAscEtd1AscIdContDestAscIdLinerAscBrokerAscIdRenbanCodeAsc(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate),
								dstCode);
			} else {
				invGenWorkPlanMstEntityList = invGenWorkPlanMstRepository
						.findByIssueInvoiceDateBetweenOrderByIssueInvoiceDateAscEtd1AscIdContDestAscIdLinerAscBrokerAscIdRenbanCodeAsc(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceFromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, invoiceToDate));
			}
		} else if (getInvoiceFromDateNullCheck(invoiceFromDate, etd1fromDate)) {
			if (checkDstCode(dstCode)) {
				invGenWorkPlanMstEntityList = invGenWorkPlanMstRepository
						.findByEtd1BetweenAndIdContDestInOrderByIssueInvoiceDateAscEtd1AscIdContDestAscIdLinerAscBrokerAscIdRenbanCodeAsc(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate),
								dstCode);
			} else {
				invGenWorkPlanMstEntityList = invGenWorkPlanMstRepository
						.findByEtd1BetweenOrderByIssueInvoiceDateAscEtd1AscIdContDestAscIdLinerAscBrokerAscIdRenbanCodeAsc(
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1fromDate),
								DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etd1ToDate));
			}
		}
	...
	private boolean getInvoiceFromDateNullCheck(String invoiceFromDate, String etd1fromDate) {
		return (StringUtils.isEmpty(invoiceFromDate) || "null".equalsIgnoreCase(invoiceFromDate.trim()))
				&& StringUtils.isNotEmpty(etd1fromDate) && !"null".equalsIgnoreCase(etd1fromDate.trim());
	}

	private boolean getEtd1AndInvoiceNullCheck(String invoiceFromDate, String etd1fromDate) {
		return StringUtils.isNotEmpty(invoiceFromDate) && !"null".equalsIgnoreCase(invoiceFromDate.trim())
				&& (StringUtils.isEmpty(etd1fromDate) || "null".equalsIgnoreCase(etd1fromDate.trim()));
	}
	...
	public List<NoemRenbanSetupMstDTO> fetchRenbanCodesByContDstCd(String contDstCode) {
		...
		List<NoemRenbanSetupMstEntity> renbanCodeList = noemRenbanSetupMstRepository
				.findByIdContDstCdOrderByIdContGrpCdAsc(contDstCode);
		// Fetch list of distinct Renban code (Container group code)

		return renbanCodeList.stream()
				.map(u -> new NoemRenbanSetupMstDTO(u.getId().getContGrpCd(), u.getId().getContGrpCd())).distinct()
	 ...
	 */
	@Override
	public List<ShippingContResultDTO> shippingResults(ShippingContSearchInputDTO shippingContSearchInputDTO) {

		if (shippingContSearchInputDTO.getBookingNo().isBlank())
			shippingContSearchInputDTO.setBookingNo(null);

		if (shippingContSearchInputDTO.getRenbanCodes().isEmpty())
			shippingContSearchInputDTO.setRenbanCodes(null);

		validateShippingResultsRequest(shippingContSearchInputDTO);

		// Find data from Renban setup master based on container destination code
		List<Map<String, Object>> noemVprDlyContList = noemVprDlyContRepository.findDetails(
				shippingContSearchInputDTO.getContainerDestination(),
				DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT,
						shippingContSearchInputDTO.getEtdFrom()),
				DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT,
						shippingContSearchInputDTO.getEtdTo()),
				shippingContSearchInputDTO.getBookingNo(), shippingContSearchInputDTO.getRenbanCodes(),
				shippingContSearchInputDTO.getRenbanCodes() == null
				|| shippingContSearchInputDTO.getRenbanCodes().isEmpty() ? "Y" : "N"); // For now using this
		// flag for handling
		// empty renban codes
		// list

		return noemVprDlyContList.stream()
				.map(u -> new ShippingContResultDTO(Integer.valueOf(u.get("SR_NO").toString()),
						u.get("ETD_1") != null ? u.get("ETD_1").toString() : "",
								u.get("CONT_RENBAN_NO") != null ? u.get("CONT_RENBAN_NO").toString() : "",
										u.get("CONT_SIZE") != null ? u.get("CONT_SIZE").toString() : "",
														...
														u.get("VAN_DT") != null ? u.get("VAN_DT").toString() : "",
																u.get("INV_DATA_CAME") != null ? u.get("INV_DATA_CAME").toString() : "",
																		u.get("INV_GEN_FLG") != null ? u.get("INV_GEN_FLG").toString() : "",
																				u.get("BOOKING_NO") != null ? u.get("BOOKING_NO").toString() : "",
																						u.get("ISO_CONT_NO") != null ? u.get("ISO_CONT_NO").toString() : "",
																								u.get("SEAL_NO") != null ? u.get("SEAL_NO").toString() : ""))
	 ...
	 * @throws Exception the exception
	 */
	private void validateShippingResultsRequest(ShippingContSearchInputDTO shippingContSearchInputDTO) {
		...
		if (StringUtils.isBlank(etdFrom) || StringUtils.isBlank(etdTo) || StringUtils.isBlank(contDestination)) {
			throw new InvalidInputParametersException(ConstantUtils.ERR_CM_3001);
		}

		Date etdFromDate = DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etdFrom);
		Date etdToDate = DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, etdTo);
	 ...
	 * @throws Exception the exception
	 */
	public Map<String, Object> generateHaisenNo(NoemHaisenDtlsEntityDTO noemHaisenDtlsEntity) {
		int lintSeqNo = 0;
		...
		// if not exist - Select “Haisen code 1” from OEM_PARAMETER as prefix, then
		// assign sequence no.001
		// if exist - Get maximum existing Haisen no.
		// If existing maximum haisen no. seq doesn’t equal to 999, then assign haisen
		// seq no. + 1
		// If existing maximum haisen no. seq is equal to 999, get new haisen prefix
		// from parameter “Haisen code 2” or “Haisen code 3” in sequence, then assign
		// new haisen no. 001 (eg. TA001 or TB001
		...
		noemHaisenDtlsIdEntity.setEtdDate(
				DateUtil.dateFromStringDateFormateforInvoiceDate(DEFAULT_DATE_FORMAT, noemHaisenDtlsEntity.getEtdDate()));
		...
		if (noemHaisenDtlsRepository.findById(noemHaisenDtlsIdEntity).isEmpty()) {
			...
			if (oemParameterEntity.isPresent()) {
				...
				throw new MyResourceNotFoundException(constantProperties.getErrorHaisenNoConfigurationMissing());
			}

			haisenYearMth = formatter.format(
					LocalDate.parse(noemHaisenDtlsEntity.getEtdDate(), DateTimeFormatter.ofPattern(DEFAULT_DATE_FORMAT)));

			lstrSeqCd = "HAISEN_1";
			Optional<NoemSeqCtrlMstEntity> noemSeqCtrlMstEntity = noemSeqCtrlMstRepository
					.findById(new NoemSeqCtrlMstIdEntity(haisenYearMth, "INS", lstrSeqCd));
			...
			if (lintSeqNo == 999) {
				...
				lstrSeqCd = "HAISEN_2";
				NoemSeqCtrlMstIdEntity noemSeqCtrlMstIdEntity1 = new NoemSeqCtrlMstIdEntity(haisenYearMth, "INS",
						lstrSeqCd);
				Optional<NoemSeqCtrlMstEntity> noemSeqCtrlMstEntity1 = noemSeqCtrlMstRepository
						.findById(noemSeqCtrlMstIdEntity1);
				...
				if (lintSeqNo == 999) {
					...
					if (oemParameterEntity2.isPresent()) {
						...
						throw new MyResourceNotFoundException(
								constantProperties.getErrorHaisenNoConfigurationMissing());
					}

					lstrSeqCd = "HAISEN_3";
					NoemSeqCtrlMstIdEntity noemSeqCtrlMstIdEntity2 = new NoemSeqCtrlMstIdEntity(haisenYearMth, "INS",
							lstrSeqCd);
					Optional<NoemSeqCtrlMstEntity> noemSeqCtrlMstEntity2 = noemSeqCtrlMstRepository
							.findById(noemSeqCtrlMstIdEntity2);
					...
					if (lintSeqNo == 999) {
						throw new MyResourceNotFoundException(constantProperties.getErrorHaisenNoLimitReached());
			...
			Optional<NoemSeqCtrlMstEntity> updateNoemSeqCtrlMstEntity = noemSeqCtrlMstRepository
					.findById(new NoemSeqCtrlMstIdEntity(haisenYearMth, "INS", lstrSeqCd));
