...
public class InvHaisenDetailsSeviceImpl implements InvHaisenDetailsSevice {
	...
	public List<String> saveHaisenDetails(String userId, List<NoemHaisenDetailDTO> inputDto)
		...
		Date currentDateTime = Date.valueOf(LocalDate.now());
		...
		for (NoemHaisenDetailDTO dto : inputDto) {
			...
			idEntity.setEtdDate(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEtd()));
			...
			if (haisenNoandYrMnth.isPresent()) {
				...
				Date inputEta = DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEta());
				...
				if ((dto.getHaisenNo().equals(newHaisenNo) && dto.getHaisenYearMonth().equals(newHaisenYrMnth))) {

					List<InsInvDtlsEntity> insInvDtlsEntity = insInvDetailsRepository
							.findByIndHaisenNoAndIndHaisenYearMonth(dto.getHaisenNo(), dto.getHaisenYearMonth());

					// Updating feeder details if any change in Haisen and Invoice tables
					for (InsInvDtlsEntity invoice : insInvDtlsEntity) {

						if ((!inputFeederVessel.equals(invoice.getIndVesselNameFeeder()))
								|| (!inputFeederVoyage.equals(invoice.getIndVoyageNoFeeder()))) {
					...
					if ((!inputFeederVessel.equals(haisenentity.getVesselFeeder()))
							|| (!inputFeederVoyage.equals(haisenentity.getVoyageFeeder()))
							|| (inputEtaLd.compareTo(haisenentity.getEtaDate()) != 0)) {
				...
				if (!isSaved) {
					...
					List<String> invNoLinkedToHaiseList = insInvDetailsRepository.getInvNo(dto.getHaisenNo(),
							dto.getHaisenYearMonth());
					...
					ContainerDetailsResponseDTO containerDetailsResponseDTO = getContainerDetails(newHaisenNo,
							newHaisenYrMnth);
					...
					BigDecimal containerEffeciency = new BigDecimal(
							containerDetailsResponseDTO.getContainerEfficiency());
					Integer lclVol = containerDetailsResponseDTO.getLclVolume();
					Date eta = DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEta());

					// Update the Haisen Dtls table with the above selected information
					updateHaisenDetails(newHaisenNo, newHaisenYrMnth, noOf20FtCont, noOf40FtCont, containerEffeciency,
							lclVol, userId, currentDateTime, eta);
					...
					if (!(dto.getHaisenNo().equals(newHaisenNo) && dto.getHaisenYearMonth().equals(newHaisenYrMnth))) {
						...
						updateHaisenDetails(dto.getHaisenNo(), dto.getHaisenYearMonth(), 0, 0, new BigDecimal(0), 0,
								...
								DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEta()));

						// Update the TNSO flag
						updateInvoiceTNSO(dto.getHaisenNo(), dto.getHaisenYearMonth(), newHaisenNo, newHaisenYrMnth,
								userId);
				...
				noemHaisenDtlsEntityDTO.setUpdatedDate(currentDateTime);
				...
				Map<String, Object> haisenNoResponse = invGenWorkPlanMsteService
						.generateHaisenNo(noemHaisenDtlsEntityDTO);
				...
				entity.setEtaDate(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEta()));
				...
				entity.setUpdatedDate(currentDateTime);

				noemHaisenDtlsRepository.save(entity);

				// Update the old Haisen No and Year Month details to Zero.
				updateHaisenDetails(dto.getHaisenNo(), dto.getHaisenYearMonth(), 0, 0, new BigDecimal(0), 0, userId,
						currentDateTime, DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEta()));
	...
	private void updateHaisenDetails(String haisenNo, String haisenYearMonth, Integer noOf20FtCont,
		...
		noemHaisenDtlsRepository.updateHaisenDtls(haisenNo, haisenYearMonth, noOf20FtCont, noOf40FtCont,
				containerEffeciency, lclVol, userId, currentDate, eta);
	}

	// get container information
	public ContainerDetailsResponseDTO getContainerDetails(String newHaisenNo, String newHaisenYrMnth)
	...
	// Update invoice table with the new Haisen No and Month
	public void updateInvDetails(String userId, NoemHaisenDetailDTO dto, String newHaisenNo, String newHaisenYrMnth)
		...
		for (InsInvDtlsEntity inv : insInvDtlsEntity) {
			if (inv != null) {
				if (time != null && StringUtils.isNoneBlank(dto.getEtd()) && inv.getIndInvDt() != null
						&& (inv.getIndInvDt().compareTo(time) > 0)) {
	...
	public void updateInvoiceTNSO(String oldHaisenNo, String oldHaisenyrMnth, String newHaisenNo,
			String newHaisenYrMnth, String userId) {

		List<InsInvDtlsEntity> entity = insInvDetailsRepository.getInvTnso(oldHaisenNo, oldHaisenyrMnth, newHaisenNo,
				newHaisenYrMnth);

		for (InsInvDtlsEntity invEntity : entity) {
			invEntity.setIndTnsoFlg("N");
			invEntity.setIndUpdBy(userId);
			invEntity.setIndUpdDt(currentTime);

			insInvDetailsRepository.save(invEntity);
		}

	}

	public Timestamp convertStringToTimestamp(String strDate) throws ParseException {
	...
	@Override
	public List<String> updateInvoiceDetails(String userId, List<InvoiceDetailsResponseDto> invoiceDetailsResponseDto) throws JsonProcessingException, ParseException
			  {
		List<String> response = new LinkedList<>();
		...
		for (InvoiceDetailsResponseDto dto : invoiceDetailsResponseDto) {
			...
			idEntity.setEtdDate(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEtdDate()));
			idEntity.setBuyer(dto.getBuyer());
			idEntity.setDepPort(dto.getPortOfLoading());
			idEntity.setDstPort(dto.getPortOfDischarge());
			idEntity.setVesselOcean(dto.getOceanVessel());
			...
			if (haisenNoandYrMnth.isPresent()) {
				...
				if (fieldArray[2] != null && !"null".equals(fieldArray[2])) {
					...
					existingEta = DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, formatDate);
				}
				...
				Date eta = DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEtaDate());
				...
				if (existingEta != null && eta != null && (existingEta.compareTo(eta) == 0) && existingPol != null
						&& existingPol.equals(portOfLoading) && existingPoD != null
						&& existingPoD.equals(portOfDischarge) && existingOcceanVes != null
						&& existingOcceanVes.equals(oceanVessel) && existingOcceanVoy != null
						&& existingOcceanVoy.equals(oceanVoyage)) {
				...
				// Update invoice table with the new Haisen No and Month
				updateInvoiceTable(invNo, newHaisenNo, newHaisenYrMnth, eta, oceanVessel, oceanVoyage, portOfLoading,
						portOfDischarge, userId);

				// Update the Haisen Dtls table with the above selected information
				updateHaisenforInvoiceChange(newHaisenNo, newHaisenYrMnth, eta, portOfLoading, portOfDischarge,
						oceanVessel, oceanVoyage, userId);
				...
				entity.setHaisenNo(newHaisenNo);
				entity.setHaisenYearMonth(newHaisenYrMnth);
				entity.setEtaDate(DateUtil.dateFromStringSqlDateFormate(DEFAULT_DATE_FORMAT, dto.getEtaDate()));
	...
	private void updateHaisenforInvoiceChange(String newHaisenNo, String newHaisenYrMnth, Date eta,
			String portOfLoading, String portOfDischarge, String oceanVessel, String oceanVoyage, String userId) {
		Date updDt = currentDate;
		noemHaisenDtlsRepository.updateHaisenforInvoiceChange(newHaisenNo, newHaisenYrMnth, eta, portOfLoading,
				portOfDischarge, oceanVessel, oceanVoyage, userId, updDt);

	}

	private void updateInvoiceTable(String invNo, String haisenNo, String haisenYrMnth, Date eta, String oceanVessel,
			String oceanVoyage, String portOfLoading, String portOfDischarge, String userId) {

		Date updt = currentDate;

		insInvDetailsRepository.updateInvoiceTable(invNo, haisenNo, haisenYrMnth, eta, oceanVessel, oceanVoyage,
				portOfLoading, portOfDischarge, userId, updt);
	}
