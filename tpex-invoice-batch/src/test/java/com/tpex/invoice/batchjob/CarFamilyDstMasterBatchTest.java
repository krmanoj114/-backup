...
class CarFamilyDstMasterBatchTest
{
	
	@Autowired
	private JobLauncher carFamilyDstMasterJobLauncher;
	...
	@Bean
	public JobLauncherTestUtils carFamilyDstMasterUtils(JobRepository jobRepository, JobLauncher carFamilyDstMasterJobLauncher) {
		JobLauncherTestUtils jobLauncherTestUtils = new JobLauncherTestUtils();
		jobLauncherTestUtils.setJobRepository(jobRepository);
		jobLauncherTestUtils.setJobLauncher(carFamilyDstMasterJobLauncher);
		jobLauncherTestUtils.setJob(carFamilyDstMasterUploadJob);
    ...
    {
    	Mockito.when(tpexConfigRepository.findByName("123")).thenReturn(new TpexConfigEntity(1, "123", "classpath:CarFamilyDstMasterUploadTest.json", null));
    	...
    	Mockito.when(tpexConfigRepository.findByName("car.family.dst.master.upload.error")).thenReturn(new TpexConfigEntity(1, "car.family.dst.master.upload.error", "classpath:CarFamilyDstMasterErrorTest.jrxml", null));
    	Mockito.when(tpexConfigRepository.findByName(Mockito.startsWith("jasper.report."))).thenReturn(new TpexConfigEntity(1, "1", "1", null));

		Mockito.when(carFamilyDestinationMasterRepository.findById(Mockito.any())).thenReturn(Optional.empty());
		Mockito.when(carFamilyMastRepository.countByCarFmlyCode(Mockito.any())).thenReturn(1L);
		Mockito.when(oemFnlDstMstRepository.countByFdDstCd(Mockito.any())).thenReturn(1L);

		CarFamilyDestinationMasterEntity carFamilyDestinationMasterEntity = new CarFamilyDestinationMasterEntity();
		carFamilyDestinationMasterEntity.setId(new CarFamilyDestinationMasterIdEntity("088W", "910D", "1"));
		carFamilyDestinationMasterEntity.setSrsName("HULIK");
		carFamilyDestinationMasterEntity.setCompanyCode("TMT");
		Mockito.when(carFamilyDestinationMasterRepository.save(Mockito.any())).thenReturn(carFamilyDestinationMasterEntity);
		...
		ProcessLogDtlsEntity processLogDtlsEntity = new ProcessLogDtlsEntity(
				1, "1", "Upload Test", "Upload Test", "Upload Test", "1100001", 1, null, "TMT",null, null);
        ...
        paramsBuilder.addString("fileName", "CarFamilyDstMasterTest.xlsx");
        paramsBuilder.addString("processControlId", "1");
        
        JobExecution jobExecution = carFamilyDstMasterJobLauncher.run(carFamilyDstMasterUploadJob, paramsBuilder.toJobParameters());
    ...
    void verifyJobErrorResults() throws Exception
    {
		...
		Mockito.when(carFamilyMastRepository.countByCarFmlyCode(Mockito.any())).thenReturn(0L);
		Mockito.when(oemFnlDstMstRepository.countByFdDstCd(Mockito.any())).thenReturn(0L);
		CarFamilyDestinationMasterEntity carFamilyDestinationMasterEntity = new CarFamilyDestinationMasterEntity();
		carFamilyDestinationMasterEntity.setId(new CarFamilyDestinationMasterIdEntity("088W", "910D", "1"));
		carFamilyDestinationMasterEntity.setSrsName("HULIK");
		carFamilyDestinationMasterEntity.setCompanyCode("TMT");
		Mockito.when(carFamilyDestinationMasterRepository.findById(Mockito.any())).thenReturn(Optional.of(carFamilyDestinationMasterEntity));
        ...
        paramsBuilder.addString("batchName", "123");
        paramsBuilder.addString("companyCode", "TMT");
        paramsBuilder.addString("fileName", "CarFamilyDstMasterTestError.xlsx");
        paramsBuilder.addString("processControlId", "1");
